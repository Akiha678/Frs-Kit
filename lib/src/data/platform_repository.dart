...
///
/// 这里每个调用都是同步的。平台在编译期就进了库，函数无法在运行时改变它，所以
/// 没有什么可等待的；为了一个常量让 UI 等一个微任务纯属浪费。这也意味着这些
/// 调用不会抛出 [FrbException] —— Rust 侧没有会失败的 `Result`。
///
/// 但它们仍然要求 [initRustBridge] 已经完成：在那之前，同步调用也无话可谈。
class PlatformRepository {
  ...
  /// *原生库*编译时所针对的操作系统。
  ///
  /// 与 `dart:io` 的 `Platform.operatingSystem` 对比：构建正确的应用里两者
  /// 一致，不一致则是发现原生库过期的最快办法。
