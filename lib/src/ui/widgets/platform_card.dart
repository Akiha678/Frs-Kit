...
/// 往返之四：同步读取宿主机信息。
///
/// 这里的每个值都不经过 `Future`，因为应用运行期间平台不会变 —— 所以 controller 的
/// `onInit` 跑完时，整张卡片就已经填好了。
///
/// 有用的是 `dart` 那一行。Rust 报告它的库是针对哪个系统编译的，Dart 报告应用跑在
/// 哪个系统上，构建正确的应用里两者一致。不一致时说明原生库过期了：它是为另一个
/// target 构建的，而其他每个面板都会因此跑到错误的二进制上。
class PlatformCard extends StatelessWidget {
  ...
  /// Dart 认为当前平台是什么。
  ///
  /// 这里用 `defaultTargetPlatform` 而不是 `dart:io` 的 `Platform`，后者在 web 上
  /// 不存在，会直接把 web 构建弄坏。
